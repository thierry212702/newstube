...
const VideoCard = ({ video, onClick }) => {
  return (
    <div
      onClick={onClick}
      className="group cursor-pointer bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden hover:border-brand-500/50 transition-all hover:shadow-lg hover:shadow-brand-500/5"
    >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-slate-500 text-xs">
              <span className="flex items-center gap-1">
                <FiEye className="text-xs" /> {video.views?.toLocaleString()}
              </span>
              <span className="flex items-center gap-1">
                <FiClock className="text-xs" /> {video.duration}
              </span>
            </div>
          </div>
        </div>
